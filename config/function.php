...
function list_barang_by_ruangan3($id_ruangan)
{
    include('conn.php');
    $query = "SELECT * FROM tb_barang_elektronik WHERE status = 'nonaktif'";

    if ($id_ruangan) {
        $query .= " AND id_ruangan = '$id_ruangan'";
    }
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        $opt .= "<option value='{$row['id_barang_elektronik']}'>{$row['nama_barang']}</option>";
    }

    return $opt;
}
function list_barang_by_ruangan4($id_ruangan)
{
    include('conn.php');
    $query = "SELECT * FROM tb_barang_elektronik WHERE status = 'aktif'";
...
function list_sumber_dana(){
    include ('conn.php');
    $query = mysqli_query($con,"SELECT * FROM tb_sumber_dana ORDER BY nama_sumber_dana ASC");
    $opt = "";
    while($row = mysqli_fetch_array($query)){
        $opt .= "<option value=\"".$row['id_sumber_dana']."\">".$row['nama_sumber_dana']."</option>";
    }  
    return $opt; 
}
...
//     // Hapus "/proses/" jika ada di path
//     $path = str_replace('/process/', '/', dirname($_SERVER['SCRIPT_NAME']) . '/');
