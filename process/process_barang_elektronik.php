    ...
    // Pastikan folder upload gambar dan QR ada
    if (!is_dir($upload_dir_img)) {
        mkdir($upload_dir_img, 0755, true);
    }
    if (!is_dir($upload_dir_qr)) {
        mkdir($upload_dir_qr, 0755, true);
    }
    ...
    if (isset($_POST['tambah'])) {
        ...
        $link = base_url_qr() . 'views/detail_barang.php?id=' . urlencode(encrypt($last_id));
    ...
    if (isset($_POST['ubah'])) {
        ...
        // Buat QR Code baru
        $url = base_url_qr() . "views/detail_barang.php?id=" . urlencode(encrypt($id));
